...
const payload = computed(() => {
  return Object.entries(props.payload).map(([key, value]) => {
    ...
    const formattedValue = props.valueFormatter
      ? props.valueFormatter(value, key)
      : formatDefaultValue(value)

    return { key, value, formattedValue, itemConfig, indicatorColor }
  ...
  >
    <slot>
      ...
      <div class="grid gap-1.5">
        <div
          v-for="{ value, formattedValue, itemConfig, indicatorColor, key } in payload"
          :key="key"
          :class="
